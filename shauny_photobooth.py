...
# Modifed by shaun@oakenfull.net
...
def start_photobooth():
	...
	if config.post_online:
	  ...
	  if config.make_montage: # make the montage
		print "Creating photostrips"
		strip_path =  config.file_path + now + "-strip"
		graphicsmagick = "gm montage -tile 1x" + str(total_pics) + " -geometry +5+5 " + config.file_path + now + "*.jpg " + strip_path + ".jpg"
		os.system(graphicsmagick) #do the graphicsmagick action

		#append the footer image to the bottom
		graphicsmagick = "gm convert " + strip_path + ".jpg " + real_path + "/footer.jpg -append " +strip_path + ".jpg"
		os.system(graphicsmagick) #do the graphicsmagick action

		#create a BW version
		graphicsmagick = "gm convert " + strip_path + ".jpg -colorspace Rec709Luma " +strip_path + "bw.jpg"
		os.system(graphicsmagick) #do the graphicsmagick action

		strip_pathL = strip_path + config.stripL # used to set if bw or color
		strip_pathR = strip_path + config.stripR
		# montage the 2 strips together ready for printing.
		graphicsmagick = "gm montage -tile 2x1 -geometry +50+0 "  + strip_pathL +".jpg " + strip_pathR +".jpg " + config.file_path + now + "-final.jpg"
		os.system(graphicsmagick) #do the graphicsmagick action
	  ...
	  if config.make_gifs: # make the gifs
		if config.hi_res_pics:
			...
			os.system(graphicsmagick) #make the .gif
	  if config.printimg:
		 #code to print to CP1200
		cPrint = "lp " #print command
		print "PRINTING:"
		if config.make_montage:
		  print "montage printing"
		  cPrint = cPrint + config.file_path + now + "-final.jpg"
		os.system(cPrint)
...
while True:
	...
	input(pygame.event.get()) # press escape to exit pygame. Then press ctrl-c to exit python.
	if config.booth_trigger = "BUTTON":
		GPIO.wait_for_edge(btn_pin, GPIO.FALLING)
	elif config.booth_trigger = "TOUCH":
		wait_touch()
	else:
		print('Error: no booth trigger set in ./config.py')
		sys.exit(0) # quit Python
